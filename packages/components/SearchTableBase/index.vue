<template>
  <div class="wq_search_table_base">
    <!-- 搜索部分 -->
    <SearchSlotBox
      class="searchslotbox"
      v-if="searchFilterData"
    ...
    <div style="display: flex; padding: 8px 0">
      <template
        v-for="(item, index) in viewOperateData"
        :key="item.name + index"
      >
      ...
      <template v-if="hideOperateData?.length">
        <el-dropdown>
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <template
                v-for="(item, index) in hideOperateData"
                :key="item.name + index"
              >
...
import SearchSlotBox from './components/SearchSlotBox.vue'
...
const viewOperateData = computed(() =>
  props.operateData?.slice(0, props.maxOperateNum),
)
const hideOperateData = computed(() =>
  props.operateData?.slice(props.maxOperateNum),
)
